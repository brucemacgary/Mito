...
export default class extends Command {
	...
	public async exec(message: Message) {
		...
		const data = await fetch(encodeURI('https://nekobot.xyz/api/image?type=anal')).then(d => d.json());

		const attachment = new MessageAttachment(data.message, 'anal.png');
